...
- (RACSignal *)increaseWithStackModel:(QiCallStackModel *)model {
    ...
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        if ([model.stackStr containsString:@"+[QiCallStack callStackWithType:]"] || [model.stackStr containsString:@"-[QiLagMonitor updateCPUInfo]"]) {
            return nil;
        }
        @strongify(self);
...
- (RACSignal *)selectClsCallWithPage:(NSUInteger)page {
    ...
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        if ([db open]) {
            ...
            NSMutableArray *arr = [NSMutableArray array];
            while ([rs next]) {
                QiCallTraceTimeCostModel *model = [self clsCallModelFromResultSet:rs];
                [arr addObject:model];
                count ++;
            }
            if (count > 0) {
...
//结果封装成 model
- (QiCallTraceTimeCostModel *)clsCallModelFromResultSet:(FMResultSet *)rs {
    QiCallTraceTimeCostModel *model = [[QiCallTraceTimeCostModel alloc] init];
